...
# 日志数据库的配置(存放日志文件名的数据库)
log_host = metadata_host
log_user = metadata_user
log_password = metadata_password
log_port = metadata_port
log_file_db_name = "metadata"
log_file_table_name = "backend_logs_monitor"
log_file_table_create_cols = "id INT PRIMARY KEY AUTO_INCREMENT, " \
...
log_data_db_name = "retail"
log_data_table_name = "backend_logs"
log_data_table_create_cols = f"id int PRIMARY KEY AUTO_INCREMENT COMMENT '自增ID', " \
...
# ############# --程序运行日志的配置项-- #############
# 配置日志输出的根目录
log_root_path = "E:/Python/python-etl/logs/"
# 配置日志输出的文件名
log_name = f'pyetl-{time.strftime("%Y%m%d-%H", time.localtime(time.time()))}.log'

# ############# --JSON订单数据采集的相关配置项-- #############
# 被采集的JSON数据,在哪个文件夹
# json_data_root_path = "G:/06-002Python大数据尊享课程资料/必学部分课程资料/4-ETL/Day01/数据资料/JSON数据（订单业务）"
json_data_root_path = "G:/06-002Python大数据尊享课程资料/必学部分课程资料/4-ETL/Day01/数据资料/JSON测试"

# 写出csv的根目录配置
retail_output_csv_root_path = "G:/06-002Python大数据尊享课程资料/必学部分课程资料/4-ETL/Day01/数据资料/"
# 写出csv的文件名字配置
retail_order_output_csv_file_name = f'orders-{time.strftime("%Y%m%d-%H%M%S", time.localtime(time.time()))}.csv'
# 写出订单详情(带商品信息)模型,csv的文件名称配置
retail_order_detail_output_csv_file_name = f'order-detail-{time.strftime("%Y%m%d-%H%M%S", time.localtime(time.time()))}.csv'

# ############# --Barcode业务的相关配置项-- #############
# 写出csv的根目录配置
barcode_output_csv_root_path = "G:/06-002Python大数据尊享课程资料/必学部分课程资料/4-ETL/Day01/数据资料/"
# 写出csv的文件名字配置
barcode_order_output_csv_file_name = f'barcode-{time.strftime("%Y%m%d-%H%M%S", time.localtime(time.time()))}.csv'

# ############# --logging业务的相关配置项-- #############
# 被采集的logging数据,在哪个文件夹
log_data_root_path = "G:\\06-002Python大数据尊享课程资料\必学部分课程资料\\4-ETL\Day01\数据资料\\backend_logs"
# 写出csv的根目录配置
log_output_csv_root_path = "G:/06-002Python大数据尊享课程资料/必学部分课程资料/4-ETL/Day01/数据资料/"
# 写出csv的文件名字配置
log_order_output_csv_file_name = f'log-{time.strftime("%Y%m%d-%H%M%S", time.localtime(time.time()))}.csv'
